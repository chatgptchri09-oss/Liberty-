...
def setup_vehicle_commands(bot: commands.Bot):
    ...
    async def controllatarga(interaction: discord.Interaction, targa: str):
        ...
        await log_command(bot, LOG_CHANNEL_ID, f"🚗 {interaction.user.mention} ha controllato la targa {targa}")
    ...
    async def assicurazione(interaction: discord.Interaction, targa: str, stato: str):
        ...
        await log_command(bot, LOG_CHANNEL_ID, f"📋 {interaction.user.mention} ha {action} l'assicurazione per {targa}")
    ...
    async def modificaveicolo(interaction: discord.Interaction, targa: str, modifiche: str):
        ...
        await log_command(bot, LOG_CHANNEL_ID, f"🔧 {interaction.user.mention} ha modificato il veicolo {targa}")
    ...
    async def sequestraveicolo(interaction: discord.Interaction, targa: str):
        ...
        await log_command(bot, LOG_CHANNEL_ID, f"🚨 {interaction.user.mention} ha sequestrato il veicolo {targa}")
    ...
    async def dissequestraveicolo(interaction: discord.Interaction, targa: str):
        ...
        await log_command(bot, LOG_CHANNEL_ID, f"✅ {interaction.user.mention} ha dissequestrato il veicolo {targa}")
    ...
    async def rimuovilibretto(interaction: discord.Interaction, targa: str):
        ...
        async with aiosqlite.connect(DATABASE_NAME) as db:
            ...
            if cursor.rowcount > 0:
                ...
                await log_command(bot, LOG_CHANNEL_ID, f"🗑️ {interaction.user.mention} ha rimosso il libretto del veicolo {targa}")
            else:
                await interaction.response.send_message(f"❌ Nessun veicolo trovato con la targa **{targa}**!", ephemeral=True)
