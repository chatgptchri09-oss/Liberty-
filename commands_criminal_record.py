...
async def get_user_arrests(user_id: str):
    ...
    async with pool.acquire() as conn:
        ...
        return [(row['id'], row['nome_completo'], row['eta'], row['residenza'], row['motivo'], row['pena'], row['created_at']) for row in rows]

async def get_arrests_by_name(nome: str, cognome: str):
    nome_completo_ricerca = f"{nome} {cognome}".lower()
    pool = await database.get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT id, user_id, nome_completo, eta, residenza, motivo, pena, created_at FROM arrests")
        matching_arrests = [
            (row['id'], row['user_id'], row['nome_completo'], row['eta'], row['residenza'], row['motivo'], row['pena'], row['created_at'])
            for row in rows if row['nome_completo'].lower() == nome_completo_ricerca
        ]
...
def setup_criminal_record_commands(bot: commands.Bot):
    ...
    async def cercapersona(interaction: discord.Interaction, nome: str, cognome: str):
        ...
        async with pool.acquire() as conn:
            ...
            fines = [(row['id'], row['user_id'], row['name'], row['surname'], row['infractions'], row['fine_amount'], row['paid']) for row in rows]
