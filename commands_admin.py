...
import aiosqlite
import os
import math # Importiamo math per la funzione max (per evitare saldi negativi)
...
def setup_admin_commands(bot: commands.Bot):
    ...
    async def add_money(interaction: discord.Interaction, utente: discord.Member, importo: int):
        ...
        async with aiosqlite.connect(DATABASE_NAME) as db:
            ...
            if user_data:
                ...
                # L'utente non esiste: crea il record con il nuovo importo + saldo iniziale (20000)
                initial_bank = 20000
    ...
    async def remove_money(interaction: discord.Interaction, utente: discord.Member, importo: int):
        ...
        async with aiosqlite.connect(DATABASE_NAME) as db:
            ...
            if not user_data:
                # Se l'utente non esiste, lo inseriamo con 0 e non possiamo rimuovere l'importo
