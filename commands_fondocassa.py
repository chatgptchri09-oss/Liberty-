...
def setup_fondocassa_commands(bot):
    ...
    async def fondocassa(interaction: discord.Interaction):
        ...
        user_companies = _get_user_companies(interaction.user)
        if not user_companies:
            await interaction.response.send_message("❌ Non fai parte di nessuna compagnia.", ephemeral=True); return
        ...
        for company in user_companies:
            ...
            embed.add_field(name=f"{emoji} {company}", value=f"**${amount:,}**", inline=False)
    ...
    async def deposita_fondocassa(interaction: discord.Interaction, compagnia: str, importo: int):
        ...
        user_companies = _get_user_companies(interaction.user)
        if compagnia not in user_companies:
            await interaction.response.send_message(
                f"❌ Non fai parte della compagnia **{compagnia}**.", ephemeral=True
            ); return
        ...
        embed = discord.Embed(title="💰 Deposito Fondo Cassa", color=discord.Color.green(), timestamp=discord.utils.utcnow())
        embed.set_author(name=interaction.user.display_name, icon_url=interaction.user.display_avatar.url)
        embed.add_field(name=f"{emoji} Compagnia", value=compagnia,               inline=False)
        embed.add_field(name="\u200b",             value="\u200b",                inline=False)
        ...
        embed.add_field(name="\u200b",             value="\u200b",                inline=False)
        embed.add_field(name="👤 Depositato da",   value=interaction.user.mention, inline=False)
        embed.set_footer(text="🤠 Red Dead Redemption II — Fondo Cassa")
    ...
    # ── /preleva-fondocassa ───────────────────────────────────────────────────
    @bot.tree.command(name="preleva-fondocassa", description="[Staff] Preleva dal fondo cassa di una compagnia")
    ...
    async def preleva_fondocassa(interaction: discord.Interaction, compagnia: str, importo: int, motivazione: str = ""):
        if not has_staff(interaction):
            await interaction.response.send_message("❌ Solo lo Staff può prelevare dal fondo cassa.", ephemeral=True); return
        if importo <= 0:
            await interaction.response.send_message("❌ Importo non valido.", ephemeral=True); return
        ...
        if current < importo:
            await interaction.response.send_message(
                f"❌ Fondi insufficienti. Disponibili: **${current:,}**", ephemeral=True
            ); return
        ...
        emoji = COMPANY_EMOJI.get(compagnia, "🏢")
        embed = discord.Embed(title="💸 Prelievo Fondo Cassa", color=discord.Color.orange(), timestamp=discord.utils.utcnow())
        embed.set_author(name=interaction.user.display_name, icon_url=interaction.user.display_avatar.url)
        embed.add_field(name=f"{emoji} Compagnia",  value=compagnia,               inline=False)
        embed.add_field(name="\u200b",              value="\u200b",                inline=False)
        ...
        embed.add_field(name="\u200b",              value="\u200b",                inline=False)
        embed.add_field(name="👤 Prelevato da",     value=interaction.user.mention, inline=False)
        if motivazione:
            embed.add_field(name="\u200b",          value="\u200b",                inline=False)
    ...
    async def saldo_fondocassa(interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="💼 Saldo Fondi Cassa — Tutte le Compagnie",
            color=discord.Color(0xDAA520),
            timestamp=discord.utils.utcnow()
        )
        totale = 0
